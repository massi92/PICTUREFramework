...
from Framework.Algorithms.BIRCH.CFSubcluster import _CFSubcluster
from Framework.DatasetReader.DatasetReader import DatasetReader

from Framework.Algorithms.BIRCH.Snapshot import BirchSnapshot as Snapshot
from Framework.Algorithms.BIRCH.Cluster import BirchCluster as Cluster


class _CFNode:  # QUESTO CREDO CORRISPONDA AL MACROCLUSTER
    ...
    def insert_cf_subcluster(self, subcluster):
        ...
        # good to go! se sono arrivato al punto dove posso inserire allora inserisco
        # ovvero se arrivo ad un nodo che non ha subcluster figli
        else:
...
class Birch(ClassNamePrefixFeaturesOutMixin, ClusterMixin, TransformerMixin, BaseEstimator, Algorithm):
    ...
    * Roberto Perdisci
    ...
    def get_string_parameters(self):
        return 'Threshold: ' + str(self.threshold) + ', Branching Factor: ' + str(self.branching_factor) + ', Window: ' + str(self.window)
    ...
    def plot_clusters(self, axis, show_snaps = False):
        ...
        columns_values = []
        for snapshot in tqdm(self.snapshots, desc='Plotting Birch Clusters'):
    ...
    def windowed_run(self):
        while self.dataset_reader.has_n_more_samples(self.window) is True:
            self.fit(self.dataset_reader.get_n_next_samples(self.window), None, True)

    def run(self):
        self.fit(self.dataset_reader.get_dataset())

    def fit(self, X, y=None, windowed=False):
        """
        Build a CF Tree for the input data.
        If I'm correct.. the tree is rebuilt everytime
        Parameters:
        ...
        return self._fit(X=X, partial=False, windowed=windowed)

    def _fit(self, X, partial, windowed=False):
        has_root = getattr(self, "root_", None)
        # getattr is like self.has_root, but if _root is empty then it returns None
        ...
        for sample in iter_func(X):
            ...
            split_required = self.root_.insert_cf_subcluster(subcluster)
            ...
            if (self.i % self.window == 0 and self.i != 0) or (self.i + 1 % self.window == 0 and windowed):
                for cluster in self._get_leaves():
                    for centroid in cluster.centroids_:
                        clusters.append(Cluster(centroid[0]))
                self.snapshots.append(Snapshot(clusters, self.i))
